...
void PWM_voidSetDuty(PWM_config_t *ptr_userConfig,u16 copy_u16fDuty)
{
    ...
    {  
        ...
        if(copy_u16fDuty >= 0 && copy_u16fDuty <= 1023)
        {
			...
			if(CHECK_BIT(TIM_TCCR1A,TCCR1A_COM1A1) == 1 && CHECK_BIT(TIM_TCCR1A,TCCR1A_COM1B1) == 0)
			{
...
void PWM_voidGeneratePWM(PWM_config_t *ptr_userConfig, u8 copy_u8FreqInHz , u8 copy_u8DutyCycle)
{
    ...
    if(copy_u8DutyCycle >= 0 && copy_u8DutyCycle <= 100)
    {
        ...
        case TIMER0:
        {
                ...
                if(CHECK_BIT(TIM_TCCR0,TCCR0_COM00))
                {
                    // OCR0 VAL
        ...
        case TIMER1:
        {
            ...
            if(TIM1_ANTI_GLITCH == ANTI_GLITCH_ENABLE)
            {
                ...
                if(CHECK_BIT(TIM_TCCR1A,TCCR1A_COM1B1) == 1)
                {
                    ...
                    if(CHECK_BIT(TIM_TCCR1A,TCCR1A_COM1B0) == 1)
                    {
                        // Inverting 
                        // Duty Cycle Calc
            ...
            else
            {
                ...
                else if (CHECK_BIT(TIM_TCCR1A,TCCR1A_COM1A1) == 1)
                {
                        /* INVRTING CHK	*/
                    if(CHECK_BIT(TIM_TCCR1A,TCCR1A_COM1A0) == 1)
                    {
                        // Inverting 
                        // Duty Cycle Calc
                        TicksVal =  ((2*TicksVal) - (((copy_u8DutyCycle * (2*TicksVal) ) / ((u16)100))))/2; 
        ...
        case TIMER2:
        {
                ...
                // Inverting 
                if(CHECK_BIT(TIM_TCCR2,TCCR2_COM20))
                {
                    // OCR2 VAL
                    TicksVal =  (510 - (((copy_u8DutyCycle * 510 ) / ((u16)100))))/2; 
                }
